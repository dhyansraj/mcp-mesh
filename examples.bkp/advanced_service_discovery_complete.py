...
This example imports ONLY from mcp-mesh to demonstrate the clean interface.
"""
...
# Import ONLY from mcp-mesh for clean interface demonstration
...
app = FastMCP("advanced-discovery-agents")


# Example agent implementations using DUAL-DECORATOR pattern
@app.tool(
...
@app.tool(name="basic_calculator", description="Simple mathematical operations agent")
@mesh_agent(
...
async def run_demo():
    ...
    try:
        ...
        # Note: Use appropriate transport based on your MCP server setup
        print("🔧 Server ready - would run with appropriate transport")
        print("📝 Example complete - check dual-decorator pattern implementation")
