...
- **⚠️ A hand-rolled `livenessProbe` on a TypeScript provider's `/health` now restarts the pod.** `/health` used to answer 200 whatever the agent's state, so pointing liveness at it was safe by accident; with a declared `healthCheck` it answers 503 on `degraded` or `unhealthy`, and an unchanged manifest turns a transient vendor blip into a restart that cannot fix the vendor. Repoint liveness and startup at `/livez` and readiness at `/ready` — `meshctl man deployment --typescript` has the stanza.
