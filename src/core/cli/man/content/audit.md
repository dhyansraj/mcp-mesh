# Dependency Resolution Audit Trail
...
This keeps the audit table noise-free while guaranteeing that any decision worth investigating is captured. The single-candidate-evicted case is included specifically so a missing dependency that previously had a candidate is visible (recently relaxed in #547).
...
- `meshctl man tags` — Tag matching and scoring
- `meshctl man schema-matching` — Schema check details (`MCP_MESH_SCHEMA_STRICT`, modes)
- `meshctl man dependency-injection` — Full DI pipeline overview
- `meshctl man cli` — `meshctl audit` command reference
