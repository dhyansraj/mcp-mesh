...
| Image                              | Description                                        |
| ---------------------------------- | -------------------------------------------------- |
| `mcpmesh/registry:2.6.0`             | Registry service for agent discovery               |
| `mcpmesh/python-runtime:2.6.0`       | Python runtime with mcp-mesh SDK pre-installed     |
| `mcpmesh/typescript-runtime:2.6.0`   | TypeScript runtime with @mcpmesh/sdk pre-installed |
| `mcpmesh/java-runtime:2.6.0`         | Java runtime with mcp-mesh Spring Boot starter     |
| `mcpmesh/ui:2.6.0`                   | Dashboard UI server (basePath: /ops/dashboard)     |
| `mcpmesh/cli:2.6.0`                  | meshctl CLI for management                         |
...
```dockerfile
FROM mcpmesh/python-runtime:2.6.0
WORKDIR /app
...
- Registry service (`mcpmesh/registry:2.6.0`)
...
helm install greeter oci://ghcr.io/dhyansraj/mcp-mesh/mcp-mesh-agent \
  --version 2.6.0 -n team-a -f greeter/helm-values.yaml
...
helm install greeter oci://ghcr.io/dhyansraj/mcp-mesh/mcp-mesh-agent \
  --version 2.6.0 -n team-b -f greeter/helm-values.yaml
```
...
cd my-agent
docker buildx build --platform linux/amd64 -t your-registry/my-agent:v2.6.0 --push .
...
```bash
# Docker
docker run -e MCP_MESH_UI_BASE_PATH=/my/custom/path mcpmesh/ui:2.6.0
