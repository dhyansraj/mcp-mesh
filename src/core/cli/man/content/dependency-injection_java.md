...
> **Discovery caveat.** An auto-registered facade **bean** requires `@MeshService` directly on the interface. An interface that only *inherits* `@MeshService` from a super-interface is still usable as a `@MeshTool` view parameter, but is not auto-discovered as a bean.
