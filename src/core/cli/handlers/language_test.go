...
func TestDetectLanguage_PythonDirectoryRequirements(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, "requirements.txt"), []byte("mcp-mesh==3.0.0"), 0644); err != nil {
...
func TestPythonHandler_GetDockerImage(t *testing.T) {
	...
	if want := "mcpmesh/python-runtime:3.0.0"; image != want {
...
func TestTypeScriptHandler_GetDockerImage(t *testing.T) {
	...
	// scripts/bump_version.py). Hard failure, not a log — this is the assertion
	// that should have caught the 2.8.0 partial bump.
	if want := "mcpmesh/typescript-runtime:3.0.0"; image != want {
		t.Errorf("GetDockerImage() = %q, want %q", image, want)
	}
}
