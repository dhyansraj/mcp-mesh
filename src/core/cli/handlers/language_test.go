...
func TestDetectLanguage_PythonDirectoryRequirements(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, "requirements.txt"), []byte("mcp-mesh==2.6.0"), 0644); err != nil {
...
func TestPythonHandler_GetDockerImage(t *testing.T) {
	...
	if image != "mcpmesh/python-runtime:2.6" && image != "mcpmesh/python-runtime:latest" {
...
func TestTypeScriptHandler_GetDockerImage(t *testing.T) {
	...
	if image != "mcpmesh/typescript-runtime:2.6" && image != "mcpmesh/typescript-runtime:latest" {
		// Just check it's not empty - version may vary
		t.Logf("Docker image: %s", image)
	}
}
