...
// Mutate calls f(ctx, m).
func (f RegistryEventFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.RegistryEventMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.RegistryEventMutation", m)
}

// The SchemaEntryFunc type is an adapter to allow the use of ordinary
// function as SchemaEntry mutator.
type SchemaEntryFunc func(context.Context, *ent.SchemaEntryMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SchemaEntryFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SchemaEntryMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SchemaEntryMutation", m)
}
