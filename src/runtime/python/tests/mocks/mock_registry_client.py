...
from _mcp_mesh.shared.support_types import HealthStatus, MockHTTPResponse
...
class MockRegistryClient:
    ...
    def __init__(
        ...
        url: str = "http://mock-registry:8000",
    ...
    def _handle_decorator_registration_go_compatible(
    ...
    def _handle_decorator_heartbeat_go_compatible(
        self, json_data: dict[str, Any]
    ) -> MockHTTPResponse:
    ...
    def _handle_decorator_registration_legacy(
        self, json_data: dict[str, Any]
    ) -> MockHTTPResponse:
    ...
    def _handle_decorator_heartbeat_legacy(
        self, json_data: dict[str, Any]
    ) -> MockHTTPResponse:
    ...
    def _validate_decorator_request(
        self, json_data: dict[str, Any], request_type: str
    ) -> None:
        ...
        for decorator in decorators:
            ...
            for dep in decorator["dependencies"]:
                if not isinstance(dep, dict) or "capability" not in dep:
                    raise ValueError(
                        "Dependencies must be objects with 'capability' field"
                    )

    def _handle_standard_registration(
        self, json_data: dict[str, Any]
    ) -> MockHTTPResponse:
...
def create_mock_registry_client(
    ...
    return MockRegistryClient(
        config or MockRegistryConfig(), go_compatibility_mode=go_compatibility_mode
    )
