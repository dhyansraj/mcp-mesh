...
def _trigger_debounced_processing():
    ...
    """
    # Skip processing entirely in test environments to prevent slow test execution
    import os

    if os.getenv("MCP_MESH_AUTO_RUN", "true").lower() in ("false", "0", "no"):
        logger.debug(
            "🧪 Test environment detected (MCP_MESH_AUTO_RUN=false), skipping debounced processing"
        )
        return
