...
class AgentMetadata(BaseModel):
    ...
    capabilities: Annotated[list[StrictStr], Field(min_length=0)] | None = Field(
        default=None, description="List of capabilities provided by agent (0 or more)"
    )
    dependencies: (
        Annotated[list[AgentMetadataDependenciesInner], Field(min_length=0)] | None
    ) = Field(
        default=None,
        description="List of agent dependencies (0 or more) - supports both simple strings and rich objects",
    )
    health_interval: Annotated[int, Field(le=3600, strict=True, ge=1)] | None = Field(
        default=30, description="Health check interval in seconds"
    )
    timeout_threshold: Annotated[int, Field(strict=True, ge=1)] | None = Field(
        default=60, description="Timeout threshold in seconds"
    )
    eviction_threshold: Annotated[int, Field(strict=True, ge=1)] | None = Field(
        default=120, description="Eviction threshold in seconds"
    )
    version: StrictStr | None = Field(default="1.0.0", description="Agent version")
    description: StrictStr | None = Field(default=None, description="Agent description")
    tags: list[StrictStr] | None = Field(
        default=None, description="Agent tags for categorization"
    )
    security_context: StrictStr | None = Field(
        default=None, description="Security context for agent"
    )
    ...
    def agent_type_validate_enum(cls, value):
        ...
        if value not in set(["mesh_agent", "mcp_agent", "system_agent"]):
            raise ValueError(
                "must be one of enum values ('mesh_agent', 'mcp_agent', 'system_agent')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "dependencies": (
                    [
                        AgentMetadataDependenciesInner.from_dict(_item)
                ...
                "health_interval": (
                    obj.get("health_interval")
                    if obj.get("health_interval") is not None
                    else 30
                ),
                "timeout_threshold": (
                    obj.get("timeout_threshold")
                    if obj.get("timeout_threshold") is not None
                    else 60
                ),
                "eviction_threshold": (
                    obj.get("eviction_threshold")
                    if obj.get("eviction_threshold") is not None
                    else 120
                ),
                "version": (
