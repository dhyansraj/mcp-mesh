...
def get_config_value(
    ...
    try:
        ...
        if source == "environment" and override is not None:
            ...
            try:
                logger.warning(f"Falling back to override value for {env_var}: {override}")
                ...
                if default is not None:
                    try:
                        logger.warning(f"Falling back to default value for {env_var}: {default}")
                        validated_default = _validate_value(default, rule, env_var)
                        return validated_default
                    except ConfigResolutionError:
                        logger.error(f"All values for {env_var} are invalid, using None")
                        return None
                else:
                    logger.error(f"Override and default values for {env_var} are invalid, using None")
                    return None
