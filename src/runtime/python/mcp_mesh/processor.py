...
from mcp_mesh.generated.registry_client.mcp_mesh_registry_client.models.mesh_tool_registration import (
...
# Since we're using the generated client directly
...
class MeshToolProcessor:
    ...
    def __init__(self, registry_client: AgentsApi) -> None:
        self.registry_client: AgentsApi = registry_client
    ...
    async def process_tools(
        ...
        try:
            ...
            if GENERATED_CLIENT_AVAILABLE:
                from mcp_mesh.generated.registry_client.mcp_mesh_registry_client.models.mesh_tool_dependency_registration import (
    ...
    async def _register_with_mesh_registry(
        ...
        """
        Send registration data to mesh registry using generated client.
        ...
        try:
            ...
            # Convert dict to MeshAgentRegistration model
            mesh_registration = MeshAgentRegistration(**registration_data)

            # Use generated client register_agent method
            response = await self.registry_client.register_agent(mesh_registration)

            # Convert response to dict for compatibility
            response_dict = response.model_dump(mode="json")
            self.logger.debug(f"✅ Registration successful: {response_dict}")
    ...
    async def _send_heartbeat_request(
        ...
        """
        Send heartbeat data to mesh registry using the generated client.
        ...
        try:
            ...
            # Convert dict to MeshAgentRegistration model
            mesh_heartbeat = MeshAgentRegistration(**heartbeat_data)

            # Use generated client send_heartbeat method
            response = await self.registry_client.send_heartbeat(mesh_heartbeat)
    ...
    async def _update_agent_registration_with_http_endpoint(
        ...
        try:
            ...
            # Call the registry to update the agent's endpoint via re-registration
            try:
                # Convert to MeshAgentRegistration for re-registration
                mesh_registration = MeshAgentRegistration(**update_data)
                response = await self.registry_client.register_agent(mesh_registration)
                ...
                if response:
                    ...
                    self.logger.warning(f"⚠️ Failed to update {agent_id} HTTP endpoint")
...
class MeshAgentProcessor:
    ...
    def _build_heartbeat_payload(self, health_status: HealthStatus) -> dict[str, Any]:
        """
        Build heartbeat payload using MeshAgentRegistration format.

        Args:
            health_status: Health status object

        Returns:
            Dictionary compatible with MeshAgentRegistration model
        """
        return {
            ...
            "http_host": health_status.metadata.get("http_host", "0.0.0.0"),
            "http_port": health_status.metadata.get("http_port", 0),
            "tools": [],  # Empty for heartbeat, tools are registered separately
            "registered_at": health_status.timestamp.isoformat(),
            "status": (
    ...
    async def _send_heartbeat(self, agent_name: str, metadata: dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Build heartbeat payload using same format as registration
            heartbeat_data = self._build_heartbeat_payload(health_status)
            mesh_heartbeat = MeshAgentRegistration(**heartbeat_data)
            ...
            response_dict = response.model_dump(mode="json") if response else None

            if response_dict and response_dict.get("status") == "success":
                self.logger.info(f"💚 Heartbeat sent successfully for {agent_name}")

                # Check if dependencies have changed
                if "dependencies_resolved" in response_dict:
                    current_deps = response_dict.get("dependencies_resolved", {})
...
class DecoratorProcessor:
    ...
    def __init__(self, registry_url: str | None = None):
        ...
        self.registry_client = AgentsApi(api_client)
    ...
    async def cleanup(self) -> None:
        ...
        if self.registry_client:
            try:
                # Close the underlying ApiClient if it has a close method
                if hasattr(self.registry_client.api_client, "close"):
                    await self.registry_client.api_client.close()
