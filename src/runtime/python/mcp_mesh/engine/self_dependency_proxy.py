...
class SelfDependencyProxy:
    ...
    def __init__(self, original_func: Callable, function_name: str):
        ...
        if not callable(original_func):
            raise ValueError(f"original_func must be callable, got {type(original_func)}")
    ...
    def __call__(self, **kwargs) -> Any:
        ...
        """
        self.logger.info(f"🔄 SELF-CALL: Direct invocation of '{self.function_name}' (bypassing HTTP)")
        self.logger.debug(f"🔄 Direct call args: {kwargs}")
        
        try:
            result = self.original_func(**kwargs)
            self.logger.info(f"✅ SELF-CALL: Direct call to '{self.function_name}' succeeded")
            self.logger.debug(f"✅ Direct call result: {type(result)} {result}")
            return result
        except Exception as e:
            self.logger.error(f"❌ SELF-CALL: Direct call to '{self.function_name}' failed: {e}")
            raise RuntimeError(f"Self-dependency call to '{self.function_name}' failed: {e}")
