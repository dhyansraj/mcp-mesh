...
class SSEParser:
    ...
    @staticmethod
    def parse_sse_response(response_text: str, context: str = "unknown") -> Dict[str, Any]:
        """
        ...
        logger.debug(f"🔧 Response text length: {len(response_text)}, starts with 'event:': {response_text.startswith('event:')}")
        ...
        is_sse_format = (response_text.startswith("event:") or 
        ...
        if not is_sse_format:
            ...
            logger.debug(f"🔧 {context}: Response preview: {repr(response_text[:200])}...")
            try:
                ...
                logger.error(f"🔧 {context}: Invalid response content (first 500 chars): {repr(response_text[:500])}")
        ...
        logger.debug(f"🔧 {context}: Found {data_line_count} data lines, accumulated JSON length: {len(accumulated_json)}")
        ...
        try:
            ...
            logger.debug(f"🔧 {context}: SSE JSON parsed successfully! Result type: {type(result)}")
            return result
        except json.JSONDecodeError as e:
            logger.error(f"🔧 {context}: Failed to parse accumulated SSE JSON: {e}")
            logger.error(f"🔧 {context}: Accumulated data preview: {accumulated_json[:500]}...")
    ...
    def parse_streaming_sse_chunk(chunk_data: str) -> Optional[Dict[str, Any]]:
        ...
        if ((chunk_data.startswith('{') and not chunk_data.endswith('}')) or
            (chunk_data.startswith('[') and not chunk_data.endswith(']')) or
...
class SSEStreamProcessor:
    ...
    def process_chunk(self, chunk_bytes: bytes) -> list[Dict[str, Any]]:
        ...
        """
        self.logger.debug(f"🌊 SSEStreamProcessor.process_chunk called for {self.context}, chunk size: {len(chunk_bytes)}")
        
        try:
            chunk_text = chunk_bytes.decode("utf-8")
            self.buffer += chunk_text
            self.logger.debug(f"🌊 {self.context}: Buffer size after chunk: {len(self.buffer)}")
        except UnicodeDecodeError:
            self.logger.warning(f"🌊 {self.context}: Skipping chunk with unicode decode error")
            return []
        ...
        self.logger.debug(f"🌊 {self.context}: Processed {events_processed} complete SSE events, yielding {len(results)} JSON objects")
        return results
