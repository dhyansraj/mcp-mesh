#!/usr/bin/env python3
"""
chat-cross-runtime-agent - Python consumer with structured output via Java Claude provider

Tests that native response_format structured output remains reliable across 5 conversation
turns in a cross-runtime scenario (Python consumer -> Java Claude provider).
"""
...
app = FastMCP("ChatCrossRuntimeAgent")
...
@mesh.tool(
    capability="chat",
    description="Chat with structured responses via cross-runtime provider",
    version="1.0.0",
...
async def chat(
    ...
    structured output works across runtimes (Python -> Java provider).
    """
    if llm is None:
...
@mesh.agent(
    name="chat-cross-runtime-agent",
    version="1.0.0",
    description="Agent testing structured output durability across cross-runtime multi-turn conversations",
    http_port=9042,
    enable_http=True,
    auto_run=True,
)
class ChatCrossRuntimeAgentConfig:
    """Agent for testing cross-runtime structured output multi-turn durability."""
