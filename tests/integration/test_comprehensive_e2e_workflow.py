...
class ProcessManager:
    ...
    def check_for_patterns(self, name: str, patterns: List[str]) -> Dict[str, List[str]]:
...
class TestComprehensiveE2EWorkflow:
    ...
    def test_01_initial_cleanup(self, test_suite):
        ...
        assert result.returncode != 0, "mcp-mesh processes should not be running after cleanup"
        
        # Verify no database files exist
        db_files = list(test_suite.project_root.glob("**/*.db"))
        assert len(db_files) == 0, f"Database files should not exist after cleanup: {db_files}"
    ...
    def test_02_start_registry(self, test_suite):
        ...
        build_result = subprocess.run(["make", "build"], cwd=test_suite.project_root, capture_output=True)
    ...
    def test_03_check_registry_logs_for_errors(self, test_suite):
        ...
        critical_errors = [e for e in errors if not any(
            acceptable in e.lower() for acceptable in [
                "no agents found",  # Expected when no agents registered yet
                "empty response"    # Expected for empty registry
            ]
        )]
        
        assert len(critical_errors) == 0, f"Registry has critical errors: {critical_errors}"
        
        # Check for successful startup patterns
        startup_patterns = test_suite.process_manager.check_for_patterns("registry", [
            r"Starting.*registry",
            r"Server.*listening",
            r"Registry.*started"
        ])
        
        assert any(startup_patterns.values()), f"Registry startup patterns not found: {startup_patterns}"
    ...
    def test_05_start_hello_world(self, test_suite):
        ...
        assert hello_world_script.exists(), f"Hello world script not found: {hello_world_script}"
        ...
        print("Waiting 1 minute for hello world agent to register and establish heartbeat...")
        time.sleep(60)
    ...
    def test_06_check_hello_world_logs(self, test_suite):
        ...
        critical_hello_errors = [e for e in hello_errors if not any(
            acceptable in e.lower() for acceptable in [
                "dependency.*not available",  # Expected when system agent not running
                "no.*provider.*found"         # Expected for unresolved dependencies
            ]
        )]
        assert len(critical_hello_errors) == 0, f"Hello world has critical errors: {critical_hello_errors}"
        ...
        critical_registry_errors = [e for e in registry_errors if not any(
            acceptable in e.lower() for acceptable in [
                "no.*provider.*found",  # Expected for unresolved dependencies
            ]
        )]
        assert len(critical_registry_errors) == 0, f"Registry has critical errors: {critical_registry_errors}"
        
        # Check for registration patterns in hello world logs
        hello_patterns = test_suite.process_manager.check_for_patterns("hello_world", [
            r"registered.*successfully",
            r"registration.*success",
            r"201"  # HTTP 201 Created response
        ])
        assert any(hello_patterns.values()), f"Hello world registration patterns not found: {hello_patterns}"
        
        # Check for heartbeat patterns in hello world logs
        heartbeat_patterns = test_suite.process_manager.check_for_patterns("hello_world", [
            r"heartbeat.*sent",
            r"heartbeat.*success",
            r"200"  # HTTP 200 OK for heartbeats
        ])
        assert any(heartbeat_patterns.values()), f"Hello world heartbeat patterns not found: {heartbeat_patterns}"
    ...
    def test_07_check_hello_world_registration(self, test_suite):
        ...
        assert registration_result["success"], f"Failed to get agent registration: {registration_result}"
        assert registration_result["expected_count_match"], f"Expected 1 agent, got {registration_result['agent_count']}"
        ...
        capability_names = [cap.get("name") if isinstance(cap, dict) else cap for cap in capabilities]
        for expected_cap in expected_capabilities:
            assert expected_cap in capability_names, f"Expected capability {expected_cap} not found in {capability_names}"
        ...
        for expected_dep in expected_dependencies:
            assert expected_dep in dependencies, f"Expected dependency {expected_dep} not found in {dependencies}"
        
        print(f"✅ Hello world agent properly registered with capabilities: {capability_names}")
    ...
    def test_08_check_mcp_mesh_dev_list_hello_only(self, test_suite):
        ...
        assert "hello" in output.lower(), f"Hello world agent not found in mcp-mesh-dev list: {output}"
    ...
    def test_09_start_system_agent(self, test_suite):
        ...
        assert system_agent_script.exists(), f"System agent script not found: {system_agent_script}"
        ...
        # Wait for 1 minute as specified
        print("Waiting 1 minute for system agent to register and establish heartbeat...")
        time.sleep(60)
        
        # Verify process is still running
        system_proc = test_suite.process_manager.processes["system_agent"]
        assert system_proc.poll() is None, "System agent process should still be running"
    ...
    def test_10_check_system_agent_logs(self, test_suite):
        ...
        critical_system_errors = [e for e in system_errors if not any(
            acceptable in e.lower() for acceptable in [
                "dependency.*not available",  # May be acceptable during startup
            ]
        )]
        assert len(critical_system_errors) == 0, f"System agent has critical errors: {critical_system_errors}"
        
        # Check for registration patterns in system agent logs
        system_patterns = test_suite.process_manager.check_for_patterns("system_agent", [
            r"registered.*successfully",
            r"registration.*success", 
            r"201"  # HTTP 201 Created response
        ])
        assert any(system_patterns.values()), f"System agent registration patterns not found: {system_patterns}"
        
        # Check for heartbeat patterns in system agent logs
        heartbeat_patterns = test_suite.process_manager.check_for_patterns("system_agent", [
            r"heartbeat.*sent",
            r"heartbeat.*success",
            r"200"  # HTTP 200 OK for heartbeats
        ])
        assert any(heartbeat_patterns.values()), f"System agent heartbeat patterns not found: {heartbeat_patterns}"
    ...
    def test_11_check_hello_world_dependency_arrival(self, test_suite):
        ...
        dependency_patterns = test_suite.process_manager.check_for_patterns("hello_world", [
        ...
        assert any(dependency_patterns.values()), f"No dependency resolution patterns found in hello world logs: {dependency_patterns}"
    ...
    def test_12_check_both_agents_registration(self, test_suite):
        ...
        assert registration_result["success"], f"Failed to get agent registration: {registration_result}"
        assert registration_result["expected_count_match"], f"Expected 2 agents, got {registration_result['agent_count']}"
        ...
        assert hello_agent is not None, f"Hello world agent not found in: {[a.get('name') for a in agents]}"
        assert system_agent is not None, f"System agent not found in: {[a.get('name') for a in agents]}"
        
        # Check hello world capabilities
        hello_capabilities = hello_agent.get("capabilities", [])
        hello_cap_names = [cap.get("name") if isinstance(cap, dict) else cap for cap in hello_capabilities]
        expected_hello_caps = ["date_service", "info"]
        for cap in expected_hello_caps:
            assert cap in hello_cap_names, f"Hello world missing capability {cap}: {hello_cap_names}"
        
        # Check system agent capabilities  
        system_capabilities = system_agent.get("capabilities", [])
        system_cap_names = [cap.get("name") if isinstance(cap, dict) else cap for cap in system_capabilities]
        expected_system_caps = ["info", "date_service"]  # Based on system_agent.py
        for cap in expected_system_caps:
            assert cap in system_cap_names, f"System agent missing capability {cap}: {system_cap_names}"
        
        print(f"✅ Both agents registered - Hello: {hello_cap_names}, System: {system_cap_names}")
    ...
    def test_13_check_mcp_mesh_dev_list_both_agents(self, test_suite):
        ...
        output = list_result["stdout"]
        assert "hello" in output.lower(), f"Hello world agent not found in mcp-mesh-dev list: {output}"
        assert "system" in output.lower(), f"System agent not found in mcp-mesh-dev list: {output}"
    ...
    def test_15_check_deregistration_logs(self, test_suite):
        ...
        deregistration_patterns = test_suite.process_manager.check_for_patterns("registry", [
            r"agent.*offline",
            r"agent.*degraded", 
            r"agent.*unhealthy",
            r"heartbeat.*timeout",
        ...
        assert any(deregistration_patterns.values()), f"No deregistration patterns found in registry logs: {deregistration_patterns}"
    ...
    def test_16_check_hello_world_dependency_removal(self, test_suite):
        ...
        removal_patterns = test_suite.process_manager.check_for_patterns("hello_world", [
        ...
        assert any(removal_patterns.values()), f"No dependency removal patterns found in hello world logs: {removal_patterns}"
    ...
    def test_17_check_system_agent_health_degraded(self, test_suite):
        ...
        assert status in ["degraded", "unhealthy", "offline"], f"System agent status should be degraded/unhealthy/offline, got: {status}"
    ...
    def test_18_check_mcp_mesh_dev_list_health_updated(self, test_suite):
        ...
        assert any(indicator in output.lower() for indicator in degraded_indicators), f"No degraded status indicators found in: {output}"
