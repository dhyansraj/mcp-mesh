...
# File operations instances
@pytest.fixture
async def file_ops(temp_dir) -> AsyncGenerator[FileOperations, None]:
    """Create FileOperations instance for testing."""
    ops = FileOperations(base_directory=str(temp_dir))
    yield ops
    await ops.cleanup()


@pytest.fixture
async def file_ops_with_mock_mesh(
    temp_dir, mock_registry_client
) -> AsyncGenerator[FileOperations, None]:
