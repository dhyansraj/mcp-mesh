...
async def test_python_processor_with_go_mock():
    ...
    class TestDecoratorProcessor(DecoratorProcessor):
        ...
        async def _send_registration_request(self, agent_id: str, metadata: dict) -> dict:
            ...
            response = await self.mock_adapter.post("/agents/register_decorators", request)
            print(f"📥 Received response: {response['status']}")
            print(f"   Dependencies resolved for {len(response.get('dependencies_resolved', []))} functions")
    ...
    print("🚀 Processing decorators - this should trigger registration and dependency injection...")
    ...
    print(f"📝 Testing hello_mesh_simple function...")
    
    # Check if dependency was injected
    if hasattr(hello_simple_func, "_injected_deps"):
        print(f"✅ Dependencies injected: {list(hello_simple_func._injected_deps.keys())}")
    ...
    print(f"📝 Testing hello_mesh_typed function...")
    
    if hasattr(hello_typed_func, "_injected_deps"):
        print(f"✅ Dependencies injected: {list(hello_typed_func._injected_deps.keys())}")
    ...
    print(f"📝 Testing test_dependencies function...")
