...
async def test_go_compatible_mock():
    ...
    registration_response = await mock_client.post("/agents/register_decorators", test_request)
    ...
    hello_simple = next(r for r in resolved if r["function_name"] == "hello_mesh_simple")
    ...
    assert hello_typed["dependencies"][0]["mcp_tool_info"]["agent_id"] == "system-agent-789"
