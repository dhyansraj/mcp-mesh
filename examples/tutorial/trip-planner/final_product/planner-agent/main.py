...
class TripRequest(MeshContextModel):
    ...
    from_city: str = Field(
        default="", description="Departure city (e.g. San Francisco)"
    )
...
async def plan_trip(
    ...
    from_clause = f" departing from {from_city}" if from_city else ""
    user_text = message or (
        f"Plan a trip to {destination}{from_clause} from {dates} with a budget of {budget}."
    )

    llm_context = {"user_preferences": prefs_summary}
    if from_city:
        llm_context["from_city"] = from_city
    ...
    if specialist_tasks:
        ...
        for label, result in zip(labels, specialist_results):
            # Serialize dicts/lists as JSON to avoid Python repr with single quotes,
            # which breaks frontend JSON parsing when values contain apostrophes.
