...
The agent will start on port 9111 by default.
...
|------|------------|-------------|
| `adventure_advisor` | `adventure_advisor` | Process input using LLM |
...
Edit the `AdventureAdvisorContext` class to add fields needed for your use case:

```python
class AdventureAdvisorContext(BaseModel):
