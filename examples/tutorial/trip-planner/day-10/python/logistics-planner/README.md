...
The agent will start on port 9112 by default.
...
|------|------------|-------------|
| `logistics_planner` | `logistics_planner` | Process input using LLM |
...
Edit the `LogisticsPlannerContext` class to add fields needed for your use case:

```python
class LogisticsPlannerContext(BaseModel):
