...
```bash
# Build the image
docker build -t chat-history-agent:latest .

# Run the container
docker run -p 9109:9109 chat-history-agent:latest
```
...
```bash
# Deploy using Helm
helm install chat-history-agent oci://ghcr.io/dhyansraj/mcp-mesh/mcp-mesh-agent \
  -n mcp-mesh \
  -f helm-values.yaml \
  --set image.repository=your-registry/chat-history-agent \
