...
```bash
# Build the image
docker build -t user-prefs-agent:latest .

# Run the container
docker run -p 9105:9105 user-prefs-agent:latest
```
...
```bash
# Deploy using Helm
helm install user-prefs-agent oci://ghcr.io/dhyansraj/mcp-mesh/mcp-mesh-agent \
  -n mcp-mesh \
  -f helm-values.yaml \
  --set image.repository=your-registry/user-prefs-agent \
