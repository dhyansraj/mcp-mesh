...
```bash
# Build the image
docker build -t hotel-agent:latest .

# Run the container
docker run -p 9102:9102 hotel-agent:latest
```
...
```bash
# Deploy using Helm
helm install hotel-agent oci://ghcr.io/dhyansraj/mcp-mesh/mcp-mesh-agent \
  -n mcp-mesh \
  -f helm-values.yaml \
  --set image.repository=your-registry/hotel-agent \
