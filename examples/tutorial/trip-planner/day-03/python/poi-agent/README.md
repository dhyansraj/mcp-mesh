...
```bash
# Build the image
docker build -t poi-agent:latest .

# Run the container
docker run -p 9104:9104 poi-agent:latest
```
...
```bash
# Deploy using Helm
helm install poi-agent oci://ghcr.io/dhyansraj/mcp-mesh/mcp-mesh-agent \
  -n mcp-mesh \
  -f helm-values.yaml \
  --set image.repository=your-registry/poi-agent \
