...
/**
 * Mesh-delegated LLM agent testing parallel tool execution with OpenAI.
 *
 * <p>This example demonstrates:
 * <ul>
 *   <li>Mesh delegation to an OpenAI LLM provider — selected via tags</li>
 ...
 *      |
 *      v
 * ConsumerOpenai (@MeshLlm, mesh-delegated, parallelToolCalls=true)
 *      |
 *      +-- discovers tools from mesh (tags: "financial", "slow-tool")
 *      |
 *      +-- delegates to an OpenAI LLM provider in the mesh
 *      |        |
 ...
 * # (or use any existing OpenAI provider from the integration suites, e.g.
 * #  tests/integration/suites/uc15_parallel_tool_calls/artifacts/openai-provider-py)
 *
 ...
 * cd examples/parallel-tools/consumer-openai-java
 * mvn spring-boot:run
 *
 ...
 */
@MeshAgent(
    name = "consumer-openai-java",
    version = "1.0.0",
    description = "Mesh-delegated LLM agent testing parallel tool execution with OpenAI",
    port = 9000
)
@SpringBootApplication
public class ConsumerOpenaiApplication {

    private static final Logger log = LoggerFactory.getLogger(ConsumerOpenaiApplication.class);

    public static void main(String[] args) {
        log.info("Starting Consumer OpenAI Agent...");
        SpringApplication.run(ConsumerOpenaiApplication.class, args);
    }
    ...
    /**
     * AI-powered stock analysis with parallel tool execution using an OpenAI
     * LLM provider in the mesh.
     *
     * <p>The {@code providerSelector} pins the request to an OpenAI provider
     * via the {@code +openai} tag. The {@code parallelToolCalls = true}
