...
/**
 * Direct-mode LLM agent testing parallel tool execution with OpenAI.
 *
 * <p>This example demonstrates:
 * <ul>
 *   <li>Direct provider mode - provider = "openai/gpt-4o" (no providerSelector)</li>
 ...
 *      |
 *      v
 * DirectConsumerOpenai (@MeshLlm, direct mode, parallelToolCalls=true)
 *      |
 *      +-- discovers tools from mesh (tags: "financial", "slow-tool")
 *      |
 *      +-- calls OpenAI directly (no mesh LLM provider needed)
 *      |        |
 ...
 * OPENAI_API_KEY=... meshctl start examples/parallel-tools/direct-consumer-openai-java -d
 *
 ...
 */
@MeshAgent(
    name = "direct-consumer-openai-java",
    version = "1.0.0",
    description = "Direct-mode LLM agent testing parallel tool execution with OpenAI",
    port = 9000
)
@SpringBootApplication
public class DirectConsumerOpenaiApplication {

    private static final Logger log = LoggerFactory.getLogger(DirectConsumerOpenaiApplication.class);

    public static void main(String[] args) {
        log.info("Starting Direct Consumer OpenAI Agent...");
        SpringApplication.run(DirectConsumerOpenaiApplication.class, args);
    }
    ...
    /**
     * AI-powered stock analysis with parallel tool execution using OpenAI directly.
     *
     * <p>The {@code provider = "openai/gpt-4o"} setting uses the LLM directly
