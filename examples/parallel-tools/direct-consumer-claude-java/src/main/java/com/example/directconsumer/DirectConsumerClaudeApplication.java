...
/**
 * Direct-mode LLM agent testing parallel tool execution with Claude.
 *
 * <p>This example demonstrates:
 * <ul>
 *   <li>Direct provider mode - provider = "anthropic/claude-sonnet-4-5" (no providerSelector)</li>
 ...
 *      |
 *      v
 * DirectConsumerClaude (@MeshLlm, direct mode, parallelToolCalls=true)
 *      |
 *      +-- discovers tools from mesh (tags: "financial", "slow-tool")
 *      |
 *      +-- calls Claude directly (no mesh LLM provider needed)
 *      |        |
 ...
 * ANTHROPIC_API_KEY=... meshctl start examples/parallel-tools/direct-consumer-claude-java -d
 *
 ...
 */
@MeshAgent(
    name = "direct-consumer-claude-java",
    version = "1.0.0",
    description = "Direct-mode LLM agent testing parallel tool execution with Claude",
    port = 9000
)
@SpringBootApplication
public class DirectConsumerClaudeApplication {

    private static final Logger log = LoggerFactory.getLogger(DirectConsumerClaudeApplication.class);

    public static void main(String[] args) {
        log.info("Starting Direct Consumer Claude Agent...");
        SpringApplication.run(DirectConsumerClaudeApplication.class, args);
    }
    ...
    /**
     * AI-powered stock analysis with parallel tool execution using Claude directly.
     *
     * <p>The {@code provider = "anthropic/claude-sonnet-4-5"} setting uses the LLM directly
