#!/usr/bin/env npx tsx
/**
 * consumer-claude-ts - MCP Mesh LLM Agent (Claude)
 *
 * A MCP Mesh LLM agent that tests parallel tool execution using
 * mesh-delegated Claude provider.
 */
...
const llmTool = mesh.llm({
  ...
  // LLM Configuration - mesh delegation to Claude provider
...
const agent = mesh(server, {
  name: "consumer-claude-ts",
  version: "1.0.0",
  description: "Mesh-delegated LLM agent (Claude) testing parallel tool execution",
  httpPort: 9000,
});

console.log("consumer-claude-ts agent initialized. Waiting for mesh connections...");
