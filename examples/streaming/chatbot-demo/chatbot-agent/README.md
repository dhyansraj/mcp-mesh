...
The agent will start on port 9181 by default.
...
|------|------------|-------------|
| `chatbot_agent` | `chatbot_agent` | Process input using LLM |
...
Edit the `ChatbotAgentContext` class to add fields needed for your use case:

```python
class ChatbotAgentContext(BaseModel):
