...
class DataFormatter:
    ...
    def format_processing_result(
        ...
        return {
            "processing_summary": {
                ...
                "processed_shape": {
                    "rows": processed_shape[0],
                    "columns": processed_shape[1],
                },
            ...
            "efficiency": {
                "rows_per_second": (
                    int(processed_shape[0] / processing_time)
                ...
                "data_reduction_percent": (
                    round(
                        (
                            1
                            - (processed_shape[0] * processed_shape[1])
                            / (original_shape[0] * original_shape[1])
                        )
                        * 100,
                        2,
                    )
                    if original_shape[0] * original_shape[1] > 0
                    else 0
                ),
            },
        }
    ...
    @staticmethod
    def to_display_table(
        df: pd.DataFrame, max_rows: int = 10, max_cols: int = 10
    ) -> str:
    ...
    @staticmethod
    def export_metadata(
        df: pd.DataFrame, processing_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "agent_info": {"name": "data-processor", "version": "1.0.0"},
        }
