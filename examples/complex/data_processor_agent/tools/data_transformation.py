...
class DataTransformer:
            ...
            try:
                if operator == "eq":
                    ...
                    filtered_df = filtered_df[
                        filtered_df[column]
                        .astype(str)
                        .str.startswith(str(value), na=False)
                    ]
                elif operator == "endswith":
                    filtered_df = filtered_df[
                        filtered_df[column]
                        .astype(str)
                        .str.endswith(str(value), na=False)
                    ]
                elif operator == "in":
    ...
    def sort_data(
        self, df: pd.DataFrame, sort_columns: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def aggregate_data(
        self, df: pd.DataFrame, group_by: List[str], aggregations: Dict[str, List[str]]
    ) -> Dict[str, Any]:
        ...
        if isinstance(agg_result.columns, pd.MultiIndex):
            agg_result.columns = [
                "_".join(col).strip() for col in agg_result.columns.values
            ]
    ...
    def clean_data(self, df: pd.DataFrame, operations: List[str]) -> Dict[str, Any]:
        ...
        for operation in operations:
            if operation == "drop_duplicates":
                ...
                after_count = len(cleaned_df)
                applied_operations.append(
                    f"Removed {before_count - after_count} empty rows"
                )
                ...
                applied_operations.append(
                    f"Removed {before_count - after_count} empty columns"
                )
                ...
                applied_operations.append(
                    f"Trimmed whitespace from {len(string_cols)} string columns"
                )
                ...
                applied_operations.append(
                    f"Standardized case for {len(string_cols)} string columns"
                )
                ...
                for col in numeric_cols:
                    ...
                    if null_count > 0:
                        applied_operations.append(
                            f"Filled {null_count} null values in {col} with mean"
                        )
    ...
    def add_calculated_column(
        self, df: pd.DataFrame, column_name: str, expression: str, description: str = ""
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "+" in expression:
                ...
                if len(parts) == 2 and all(
                    part.strip() in df.columns for part in parts
                ):
            ...
            return {
                ...
                "result": self.formatter.format_processing_result(
                    ...
                    processing_time=processing_time,
                    operations=[
                        operation_desc + (f" - {description}" if description else "")
                    ],
                ),
            }
