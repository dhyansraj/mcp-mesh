...
| Max Iterations  | 5      | Maximum agentic loop iterations |
| Response Format | json   | Output format (text/json)       |
...
| ------------ | ------------ | ----------------------- |
| `analyst_py` | `analyst_py` | Process input using LLM |
...
Edit the `AnalystPyContext` class to add fields needed for your use case:

```python
class AnalystPyContext(BaseModel):
...
Edit the `AnalystPyResponse` class to define your output structure:

```python
class AnalystPyResponse(BaseModel):
