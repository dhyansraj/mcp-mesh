...
- **`a2a_consumer=true` on the agent payload** (issue #972). Each runtime flips this self-declared boolean as soon as any `@mesh.a2a_consumer` / `@A2AConsumer` / `a2aConfig` binding is registered, so downstream tooling can identify consumer agents without walking the per-tool capability list.
