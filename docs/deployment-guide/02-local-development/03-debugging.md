  ...
  "configurations": [
    {
      ...
      "type": "python",
      "request": "launch",
      "program": "${workspaceFolder}/mcp-mesh-dev",
      "args": ["start", "${file}"],
...
2. Script path: `/path/to/mcp-mesh-dev`
3. Parameters: `start your_agent.py`
...
@mesh_agent(
    ...
    dependencies=["DatabaseAgent_query"]
)
async def process_data(
    ...
    try:
        ...
        logger.debug(f"Calling DatabaseAgent_query with id={data_id}")
        result = await DatabaseAgent_query(f"SELECT * FROM data WHERE id='{data_id}'")
...
@mesh_agent(
    capability="analyzer",
    dependencies=["DataProcessor_process", "CacheAgent_get"]
)
def analyze(
    ctx: Context,
    item_id: str,
    DataProcessor_process=None,
    CacheAgent_get=None
):
    # Debug: Check which dependencies were injected
    logger.debug("Dependency injection status:")
    logger.debug(f"  DataProcessor_process: {'✓' if DataProcessor_process else '✗'}")
    logger.debug(f"  CacheAgent_get: {'✓' if CacheAgent_get else '✗'}")

    # Debug: Log dependency metadata
    if hasattr(DataProcessor_process, '_mesh_metadata'):
        logger.debug(f"  DataProcessor metadata: {DataProcessor_process._mesh_metadata}")
```
...
@server.tool()
@mesh_agent(capability="timed_service")
@debug_timing
def slow_operation(ctx: Context, size: int):
...
grep "Injecting dependency" ~/.mcp-mesh/logs/mcp-mesh.log
