...
Eviction details include `consumer_hash`, `producer_hash`, `mode` (`subset` or `strict`), and per-field `reasons` like `missing_field` or `type_mismatch`. See [Audit Trail](audit.md) for the full audit envelope.

## See Also

- [Tag Matching](tag-matching.md) — The third disambiguator, runs just before schema
- [Audit Trail](audit.md) — Inspect why a producer was chosen or evicted
- [DDDI](dddi.md) — Distributed Dynamic Dependency Injection overview
- [Architecture](architecture.md) — System overview
- [Registry](registry.md) — Registration and event log
