...
**What this fixes**: loop-bound resources work as expected when created in `lifespan` startup. `asyncpg.Pool`, `redis.asyncio.Redis`, and `aiohttp.ClientSession` are all loop-affine — they bind to the asyncio loop that created them and fail if reused from a different one. Because v3.0.0 runs your `lifespan` AND your tools on the same user loop, the canonical FastMCP idiom is straightforward:
...
`lifespan` exit. Since v3.0.0, `lifespan` and all tool bodies share
