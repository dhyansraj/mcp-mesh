...
func createAgentCommand(agentPath string, env []string, workingDir, user, group string) (*exec.Cmd, error) {
	...
	// Create command with auto-enhancement injection
	// Use -c to safely import mcp_mesh_runtime before running the script
	enhancementCode := `
try:
    import mcp_mesh_runtime
except ImportError as e:
    print(f"Warning: mcp_mesh_runtime not available: {e}")
exec(open('` + scriptPath + `').read())
`
	cmd := exec.Command(pythonExec, "-c", enhancementCode)
	cmd.Env = env
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	cmd.Stdin = os.Stdin
