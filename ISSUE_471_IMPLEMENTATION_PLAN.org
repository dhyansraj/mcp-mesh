...
** DONE Phase 8: Fix StoreDependencyResolutions (Critical) [2026-01-23]
...
** DONE Phase 10: Improve Semver Support
   CLOSED: [2026-01-23]
   - Implemented in Phase 9 as part of Matcher.MatchVersion()
...
** DONE Phase 11: Refactor ent_service.go (2686 lines → Multiple Files)
   CLOSED: [2026-01-23]
   - Created resolver.go with resolution functions
   - ent_service.go reduced from 2686 to 2020 lines (~25% reduction)
   - matcher.go already created in Phase 9
...
** DONE Phase 12: Clean Up Repeated Anonymous Structs
   CLOSED: [2026-01-23]
   - Candidate and ScoredCandidate types introduced in Phase 9
   - No inline anonymous structs remain in core code
...
** DEFERRED Phase 13: Consolidate Dependency Types
   - Dependency and DependencySpec are similar but DependencySpec has Namespace
   - Would require changing findHealthyProviderWithTTL signature
   - Lower priority - current code works correctly
